...
class preProc():
    ...
    def normalize(self):    
        ...
        self.data_test_category = ps.DataFrame(columns = list(self.category))
        self.data_test_category[self.category] = self.data_test[self.category]
        
        self.data_train.drop(self.category, axis = 1, inplace = True)
        self.data_test.drop(self.category, axis = 1, inplace = True)
        
        self.scale = StandardScaler()
        self.scale.fit(self.data_train.values[:,:])
        self.scale.transform(self.data_train.values[:,:])
        self.scale.transform(self.data_test.values[:,:])
        ...
        self.data_train[self.category] = self.data_train_category[self.category]
        self.data_test[self.category] = self.data_test_category[self.category]