...
class preProc():
    ...
    def fit_unknowns(self, data_train, known_index, unknown_index, k, train_knn):
        ...
        for i in unknown_index:
            ...
            knn_loc = self.kNearestNeighbours(data_train.loc[i, :], data_train, k, known_index)
    ...
    def kNearestNeighbours(self, train_knn, data_train, k, known_index):
        dist = np.empty([len(known_index)+1, 2])
        index_i = 0
